...
// Server-side hard cap on the "live expedition data" context block is 60000 chars
// (see pocketbase/pb_hooks/chat.pb.js) — mirror it here so we don't ship bytes the
// server will just discard.
...
export default function ChatBot() {
  ...
  const sendFreeMessage = async (userText: string) => {
    ...
    try {
      ...
      // Fetch live data from all relevant collections in parallel. The system prompt and
      // the update_ledger_entries tool are owned server-side (see chat.pb.js) — this
      // context string is sent as untrusted reference data, not instructions, and is
      // bounded to match the server's cap.
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          context: dynamicContext.slice(0, MAX_CONTEXT_CHARS),
        }),
      })
